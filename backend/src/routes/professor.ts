...
router.get('/assignments', async (req, res) => {
  try {
    ...
    // Get assignments
    const result = await pool.query(
      `SELECT id, title, description, due_date, points, created_at, updated_at
       FROM assignments
       WHERE course_id = $1
...
router.post('/assignments', async (req, res) => {
  try {
    ...
    // Create assignment
    const result = await pool.query(
      `INSERT INTO assignments (title, description, course_id, due_date, points)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [title, description || null, courseId, dueDate || null, points || 100]
    );
